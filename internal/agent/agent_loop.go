...
func (al *agentLoop) loop(msg messagebus.Message) error {
	...
	if isNewSessionCommand(msg.Message) {
		al.ingestSessionMemory(currentSession, config.MemoryWindow)
...
// for 5W1H+R summarization and graph storage. Errors are logged but not propagated
// to avoid blocking the /new command.
func (al *agentLoop) ingestSessionMemory(currentSession session.Session, memoryWindow int) {
	if al.context.MemoryService == nil || !al.context.MemoryEnabled {
		return
	}
	messages := currentSession.GetMessages(memoryWindow)
	if len(messages) == 0 {
		return
	}
