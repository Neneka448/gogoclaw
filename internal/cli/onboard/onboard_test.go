...
func TestOnboardCreatesWorkspaceBootstrapFiles(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(workspacePath, "sqlite-vec", "store.db")); !os.IsNotExist(err) {
		t.Fatalf("sqlite-vec store should not be created during onboard, got err = %v", err)
	}
}
