...
Before any tmux operation, verify the OS and binary availability:
...
# 1. Check OS (tmux is macOS/Linux only)
if [ "$OS" != "Darwin" ] && [ "$OS" != "Linux" ]; then
  echo "OS_UNSUPPORTED: $OS (tmux requires macOS or Linux)"
  exit 0
fi
...
**If `ENV_OK`**: proceed with the tmux operation.

**If `OS_UNSUPPORTED`**: tell the user this skill requires macOS or Linux. Ask if they want to try an alternative approach (e.g., using `screen` on their platform, or running commands directly).

**If `ENV_MISSING`**: tell the user `tmux` is not installed. Ask if they want to install it:
...
Do NOT install automatically — always get user confirmation first.
