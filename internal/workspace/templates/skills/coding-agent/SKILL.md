...
Before delegating any coding task, check which coding agents are available:
...
if [ ${#AVAILABLE[@]} -gt 0 ]; then
  echo "AGENTS_OK: ${AVAILABLE[*]}"
  if [ ${#MISSING[@]} -gt 0 ]; then
    echo "AGENTS_MISSING: ${MISSING[*]}"
  fi
else
  echo "AGENTS_NONE"
fi
...
**If `AGENTS_OK`**: use one of the available agents. Prefer the agent the user requested, or default to the first available.

**If `AGENTS_NONE`**: tell the user no coding agents are installed. Ask if they want to install one:
...
Do NOT install automatically — always get user confirmation first. If the user declines, offer to handle the coding task directly instead of delegating.
