...
3. Mark the task as running:
   ```bash
   python -m skills.invoke-agent.scripts.task_start --invocation-dir {invocation_dir}
   ```
4. Execute the task. Write any output or artifacts under the invocation directory.
5. When finished successfully:
   ```bash
   python -m skills.invoke-agent.scripts.task_complete --invocation-dir {invocation_dir}
   ```
6. If you encounter an unrecoverable error:
   ```bash
   python -m skills.invoke-agent.scripts.task_fail --invocation-dir {invocation_dir} --reason "description"
   ```
